...
public class ReaderUtilClient {
    ...
    public static void main(String[] args) {
        final ReaderUtil readerUtil = new ReaderUtil((PooledObject<StringBuffer>) new GenericObjectPool<StringBuffer>(new StringBufferFactory()));
        for(int i = 0 ;i < 20; i++){
        	final Reader reader = new StringReader("foo " + i );
        	new Thread(new Runnable(){

				public void run() {
					// TODO Auto-generated method stub
					 try {
				            System.out.println(Thread.currentThread().getName() + " : " +readerUtil.readToString(reader));
				        } catch (IOException e) {
				            e.printStackTrace();
				        }
				}
